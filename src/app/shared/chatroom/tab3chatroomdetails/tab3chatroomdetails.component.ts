...
@Component({
  selector: 'app-tab3chatroom-details',
  templateUrl: './tab3chatroomdetails.component.html',
  styleUrls: ['./tab3chatroomdetails.component.scss'],
  standalone: true,
...
export class Tab3ChatroomDetailsComponent implements OnInit {
  ...
  retrieveChatroomTitle(): void {              //Retrieving title from the chatroom by ID
    ...
    const chatroomTitleRef = ref(db, `tab3/chatrooms/${this.chatroomId}/title`);
  ...
  retrieveMessages(): void {               // Retrieving messages from chatroom by ID
    const db = getDatabase();
    const messagesRef = ref(db, `tab3/chatrooms/${this.chatroomId}/messages`);
