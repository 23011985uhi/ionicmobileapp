...
@Component({
  selector: 'app-tab1chatroom-details',
  templateUrl: './tab1chatroomdetails.component.html',
  styleUrls: ['./tab1chatroomdetails.component.scss'],
  standalone: true,
...
export class Tab1ChatroomDetailsComponent implements OnInit {
  ...
  retrieveChatroomTitle(): void {              //Retrieving title from the chatroom by ID
    ...
    const chatroomTitleRef = ref(db, `tab1/maths/accmaths/chatrooms/${this.chatroomId}/title`);
  ...
  retrieveMessages(): void {               // Retrieving messages from chatroom by ID
    const db = getDatabase();
    const messagesRef = ref(db, `tab1/maths/accmaths/chatrooms/${this.chatroomId}/messages`);
