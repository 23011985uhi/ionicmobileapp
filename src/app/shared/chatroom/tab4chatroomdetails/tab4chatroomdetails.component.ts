...
@Component({
  selector: 'app-tab4chatroom-details',
  templateUrl: './tab4chatroomdetails.component.html',
  styleUrls: ['./tab4chatroomdetails.component.scss'],
  standalone: true,
...
export class Tab4ChatroomDetailsComponent implements OnInit {
  ...
  retrieveChatroomTitle(): void {              //Retrieving title from the chatroom by ID
    ...
    const chatroomTitleRef = ref(db, `tab4/chatrooms/${this.chatroomId}/title`);
  ...
  retrieveMessages(): void {               // Retrieving messages from chatroom by ID
    const db = getDatabase();
    const messagesRef = ref(db, `tab4/chatrooms/${this.chatroomId}/messages`);
